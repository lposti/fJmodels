...
#pragma omp parallel for
		...
		fscanf(ofile,"%d %d %d",&nr,&npoly,&ngauss);
...
// distribution of density on shell radius ar[n]
	...
	for(int np=0;np<npoly;np++) for (int n=0;n<nr;n++) 	printf("%d %d %f %f\n",np,n,I_ext[n][np],I_int[n][np]);
	for(int np=0; np<npoly; np++){
		int l=2*np;
		if(l==0){
	...
	FILE* rhof=fopen("rhof.dat","w");
	for (int i=0;i<nr;i++) fprintf(rhof,"%e %e %e %e %e %e\n",ar[i],rho[i][0],evpot(ar[i]*ar[i],0),phi_Hern(ar[i]),rho_Hern(ar[i]),phil[i][0]);
	fclose(rhof);
...
// npoly coeffs for Gauss integration over angles ----------------------------
	...
	if(rd==0){//compute, don't read data
		...
		if(ofile==NULL) printf("cannot open file %s\n",fname);
		rho[0][0]=(*dens)(.01*ar[1],0,Vbar[0],sigR[0],sigp[0],sigz[0],sigRz[0]);
...
#pragma omp parallel for
		...
		rhl[0][0]=2*rho[0][0];
		for(int i=1;i<ngauss;i++) rhl[0][i]=0;
		if(prnt==1) printf("\n");
		fprintf(ofile,"%d %d %d\n",nr,npoly,ngauss);
		compress(ofile,ar,nr); compres2(ofile,rhl,nr,npoly);
	...
	for(int np=0; np<npoly; np++){
		...
		for(int n=1; n<nr; n++){
			...
			Pr2[n][np]=-TPI*G*((l+2)*(l+1)*I_int[n][np]/pow(ar[n],l+3)+
					   (l-1)*l*(I_ext[nr-1][np]-I_ext[n][np])*pow(ar[n],l-2)
					   -(2*l+1)*rhl[n][np]);
			printf("phi[%d][%d]=%f %f\n",n,np,phil[n][np],rho[n][np]);
		}
	}
	// debug purposes
	//for (int n=0;n<nr;n++) printf(">> phil[%d][0]=%f phil[%d][1]=%f phil[%d][2]=%f\n",n,phil[n][0],n,phil[n][1],n,phil[n][2]);
	delmatrix(poly,npoly); delmatrix(rho,nr);
