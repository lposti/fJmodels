...
inverse matrix will be scrambled by columns. */
	if(irow != icol) {
            for(l=0; l<n; l++)
		WDswap(a[irow][l], a[icol][l]);
            for(l=0; l<m; l++)
		WDswap(b[irow][l], b[icol][l]);
	}
	indxr[i]=irow;
	indxc[i]=icol;
...
was built up. */
	...
	if (indxr[l] != indxc[l] )
	    for(k=0; k<n; k++)
		WDswap(a[k][indxr[l]], a[k][indxc[l]]);
    }
    delete[] ipiv;
    delete[] indxr;
    delete[] indxc;
...
/* Gaussian eleWDmination with backsubstitution (cf. Numerical Recipes sect. 2.2).
...
/* Partial pivoting: the biggest element in the column i, in and below row i */
	irow = i;
	for(j=i+1; j<n; j++)
	    if(abs(a[j][i]) > abs(a[irow][i])) irow = j;
        if(a[irow][i] == 0.)
...
------------------------------------------------------------------------------*/
{
    ...
    v = tab*eta;
    if(err<v) err = v;
    if(m==mx) Numerics_error("qbulir exceeding WDmaximum of iterations");
    return c;
}
////////////////////////////////////////////////////////////////////////////////
	      ...
	      const double tol)
{
	...
	if(abs(e)>=tol1 && abs(fa)>abs(fb)) {
	    ...
	    if(p>0.) q =-q;
            ...
            if(2.*p < WDmin(3.*xm*q-abs(tol1*q), abs(e*q))) {
		e = d;
		d = p/q;
	    } else {
		d = xm;
             ...
             const float tol)
{
	...
	if(abs(e)>=tol1 && abs(fa)>abs(fb)) {
	    ...
	    if(p>0.f) q =-q;
            ...
            if(2.f*p < WDmin(3.f*xm*q-abs(tol1*q), abs(e*q))) {
		e = d;
		d = p/q;
	    } else {
		d = xm;
...
int LUDecomposition(PPdbl a, const int n, Pint indx, int& d)
{
    ...
    for(i=0; i<n; i++) {
        big = 0.;
	for(j=0; j<n; j++) big = WDmax(big, abs(a[i][j]));
...
void elmhes(double** a, const int n)
{
	...
	if(i!=m) {
	    for(j=m-1; j<n; j++) WDswap(a[i][j],a[m][j]);
	    for(j=0;   j<n; j++) WDswap(a[j][i],a[j][m]);
	}
	if(x) {
	    for(i=m+1; i<n ;i++)
		if((y=a[i][m-1]) != 0.) {
...
{
    register int    nn,m,l,k,j,its,i,mWDmin;
