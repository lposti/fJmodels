...
int topbottom(double *xm,int n,double x,int *botx,int *topx){
	int m, top=n-1, bot=0;
	if((xm[top]-x)*(x-xm[bot])<0.){
...
int topbottom(float *xm,int n,float x,int *botx,int *topx){
    int m, top=n-1, bot=0;
    if((xm[top]-x)*(x-xm[bot])<0.){
        if(fabs(xm[top]-x)<1e-5*fabs(x-xm[bot])){
            *topx=top; *botx=top-1; return 1;
...
float linterp2d(float **f,float *xm,int nx,float *ym,int ny,
         float x,float y){ // 2d linear interpolation
    int topx,botx,topy,boty;
    topbottom(xm,nx,x,&botx,&topx);
    topbottom(ym,ny,y,&boty,&topy);
    float dx=(x-xm[botx])/(xm[topx]-xm[botx]);
    float dy=(y-ym[boty])/(ym[topy]-ym[boty]);
    float fa=f[botx][boty]*(1-dx)+f[topx][boty]*dx;
    float fb=f[botx][topy]*(1-dx)+f[topx][topy]*dx;
    return fa*(1-dy)+fb*dy;
}
...
void rkck(double *par,double y[], double dydx[],int N, double x, double h, double yout[],
	  double yerr[], void (*derivs)(double*,double, double [], double []))
{//version with carried parameters
	int i;
	static double a2=0.2,a3=0.3,a4=0.6,a5=1.0,a6=0.875,b21=0.2,
	b31=3.0/40.0,b32=9.0/40.0,b41=0.3,b42 = -0.9,b43=1.2,
	b51 = -11.0/54.0, b52=2.5,b53 = -70.0/27.0,b54=35.0/27.0,
	...
	for (i=0;i<N;i++)
		ytemp[i]=y[i]+b21*h*dydx[i];
	(*derivs)(par,x+a2*h,ytemp,ak2);
	for (i=0;i<N;i++)
		ytemp[i]=y[i]+h*(b31*dydx[i]+b32*ak2[i]);
	(*derivs)(par,x+a3*h,ytemp,ak3);
	for (i=0;i<N;i++)
		ytemp[i]=y[i]+h*(b41*dydx[i]+b42*ak2[i]+b43*ak3[i]);
	(*derivs)(par,x+a4*h,ytemp,ak4);
	for (i=0;i<N;i++)
		ytemp[i]=y[i]+h*(b51*dydx[i]+b52*ak2[i]+b53*ak3[i]+b54*ak4[i]);
	(*derivs)(par,x+a5*h,ytemp,ak5);
	for (i=0;i<N;i++)
		ytemp[i]=y[i]+h*(b61*dydx[i]+b62*ak2[i]+b63*ak3[i]+b64*ak4[i]+b65*ak5[i]);
	(*derivs)(par,x+a6*h,ytemp,ak6);
	for (i=0;i<N;i++)
		yout[i]=y[i]+h*(c1*dydx[i]+c3*ak3[i]+c4*ak4[i]+c6*ak6[i]);
	for (i=0;i<N;i++)
		yerr[i]=h*(dc1*dydx[i]+dc3*ak3[i]+dc4*ak4[i]+dc5*ak5[i]+dc6*ak6[i]);
	  ...
	  void (*derivs)(double, double*, double*))
{
	...
	for (;;) {
		...
		if (xnew == *x) 
		{
			printf("At x= %f stepsize underflow in rkqs",xnew);
			exit(0);
		}
	}
...
void rkqs(double *par,double *y,double *dydx,int N,double *x,double htry,double eps,
	  double *yscal, double *hdid, double *hnext,
	  void (*derivs)(double*,double, double*, double*))
{//version for when parameters are required
	int i;
	double errmax,h,htemp,xnew,yerr[6],ytemp[6];
	h=htry;
	for (;;) {
		rkck(par,y,dydx,N,*x,h,ytemp,yerr,derivs);
		errmax=0.0;
		for (i=0;i<N;i++) errmax=MAX(errmax,fabs(yerr[i]/yscal[i]));
		errmax /= eps;
		if (errmax <= 1.0) break;
...
double zbrent(double (*func)(double), double x1, double x2,double fa,double fb,double tol,int itmax){
	int iter;
...
//version of the above for functions that have parameters: store
//parameters in pars[]
...
void sort(unsigned long n, double *arr)
{
	...
	int *istack = new int[NSTACK]; istack-=1;
	for (;;) {
		if (ir-l < M) {
			for (j=l+1;j<=ir;j++) {
				a=arr[j];
...
void gauleg(double x1, double x2, double *x, double *w, int n){
	int m=(n+1)/2;
	double xm=0.5*(x2+x1), xl=0.5*(x2-x1), pp, z1;
	for (int i=0;i<m;i++) {
		double z=cos(3.141592654*(i+0.75)/(n+0.5));
		do {
			double p1=1, p2=0, p3;
			for (int j=0;j<n;j++) {
				p3=p2;
				p2=p1;
				p1=((2*(j+1)-1)*z*p2-j*p3)/(double)(j+1);
			}
